# 🧩 Mini Project – Excel Basic
This folder contains **basic-level Excel mini projects** focused on building fundamental skills in data handling, calculation, and simple analysis. The projects in this section emphasize **accuracy, clarity, and correct use of core Excel functions**.

## 🛠 Tools Used
- Microsoft Excel

## 📁 Dataset
- Dataset Sales in One month total 51 Data
- Projects use simple and dummy datasets for practice
- The dataset used is in the portfolio file.
  
## 🎯 Project Focus
The Basic level projects aim to practice:
- Fundamental Excel formulas and functions
- Basic data formatting and organization
- Simple data cleaning techniques
- Introductory data analysis and visualization

## 📘 Key Skills Practiced
- Basic Functions (e.g., `SUM`, `AVERAGE`, `COUNT`)
- Date & Time Functions
- Text and Math Functions
- Logical Functions (`IF`; `SUMIF`)
- Conditional Formatting
- Basic Charts

## 🧹 Data Preparation
Typical preparation steps include:
- Identifying blank or missing values
- Removing duplicate records
- Standardizing data formats
- Ensuring data consistency

## 📊 Project Output & Analysis
The output of this mini project includes **simple analytical reports and visualizations**, such as:
- **Total Values Summary**
  - Displays aggregated results (e.g., total sales, quantities, or counts).
  - Total Sales = Rp 14.005.000
  - Avg Total Sales = Rp 274.608
  - Total Order Id = 51

- **Total Sales per Region**
  - Total sales or purchases were highest in the ‘East’ region with total revenue of Rp 3,360,000, followed by the ‘West’, ‘South’, and ‘North’ regions <br>
    <img width="441" height="383" alt="image" src="https://github.com/user-attachments/assets/19d1c745-8af9-429f-a2db-3b66e44f23ea" />

- **Total Sales Over Time**
  - The highest sales were on January 1 and 25, with total revenue of Rp 750,000. <br>
    <img width="566" height="443" alt="image" src="https://github.com/user-attachments/assets/81ec75c8-c880-4387-8666-e1702ecdfb0a" />

- **Total Sales per Category**
  - The category with the highest sales came from ‘Footwear’, worth IDR 5,000,000, and the lowest from ‘Electronics’ with a value of IDR 2,000,000. <br>
    <img width="500" height="435" alt="image" src="https://github.com/user-attachments/assets/1d898a75-7a6a-4755-8d6d-dd81f3e8a2a3" />
    
- **Sales High vs Low**
  - High-priced products dominate sales compared to low-priced products. This means that customers are more likely to repurchase high-priced products or make bulk purchases (purchasing many items at once).<br>
    <img width="444" height="441" alt="image" src="https://github.com/user-attachments/assets/47822675-0e5e-4858-b385-d9b91616445a" />
  
## ✅ Status
✔ Completed
